...
CREATE TABLE IF NOT EXISTS `testdb`.`event` (
                                              ...
                                              `description` LONGTEXT NULL,
                                              `status` ENUM('upcoming', 'in progress', 'done') NOT NULL DEFAULT 'upcoming',
    PRIMARY KEY (`id`),
    UNIQUE INDEX `id_UNIQUE` (`id` ASC) ,
...
-- -----------------------------------------------------
-- Table `testdb`.`ip_address`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `testdb`.`ip_address` (
                                                   `user_id` INT NULL,
                                                   `country` VARCHAR(255) NULL,
    `region_name` VARCHAR(255) NULL,
    `city` VARCHAR(255) NULL,
    INDEX `fk_ip_address_user1_idx` (`user_id` ASC) ,
    CONSTRAINT `fk_ip_address_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `testdb`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
